...
class SingleDefaultEnumRuleTest {
	...
	def void checkValid() {
		"
			robot UnitTestRobot {
				datatype enum TestEnum from integer to {
					default -> unit
				}
			}
	...
	@Test
	def void checkErrorMultipleDefaultCases() {
		"
			robot UnitTestRobot {
				datatype enum TestEnum from integer to {
					default -> unit_one
		...
		".parse.assertError(
			...
			SingleDefaultEnumRule.MULTIPLE_DEFAULT_CASES
		)
	}

	@Test
	def void checkErrorNoDefaultCase() {
		"
			robot UnitTestRobot {
				datatype enum TestEnum from integer to {
				}
		...
		".parse.assertError(
			RobotSupervisoryControllerDSLPackage.Literals.ENUM_DATA_TYPE,
			SingleDefaultEnumRule.NO_DEFAULT_CASE
		)
	}
}
