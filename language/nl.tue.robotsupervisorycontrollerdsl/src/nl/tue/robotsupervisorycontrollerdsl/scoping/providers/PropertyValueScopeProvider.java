...
public class PropertyValueScopeProvider extends AbstractScopeProvider {
	...
	public List<EObject> determineCandidates(EObject context, EReference reference) {
		...
		ObjectPropertyValue parentPropertyValue = ModelHelper.findParentOfType(context.eContainer(), ObjectPropertyValue.class);
		...
		if (dataType instanceof ComplexDataTypeReference) {
			...
			return ModelHelper.findChildren(typeReference.getType(), ObjectProperty.class)
					.stream()
					.map(it -> (EObject) it)
	...
	public boolean supports(EObject entity, EReference reference) {
		return entity instanceof ObjectValue && reference == RobotSupervisoryControllerDSLPackage.Literals.OBJECT_PROPERTY_VALUE__PROPERTY;
	}
