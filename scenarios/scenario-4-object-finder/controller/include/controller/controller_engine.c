...
const char *controller_event_names[] = {
    ...
    "component_ObjectDetector.c_pFDUHMFYFEZ1C", /**< Event component_ObjectDetector.c_pFDUHMFYFEZ1C. */
    "component_ObjectDetector.c_pIT6QHH818CVU", /**< Event component_ObjectDetector.c_pIT6QHH818CVU. */
    ...
    "data_move.c_p1HIPH7IC9U78",                /**< Event data_move.c_p1HIPH7IC9U78. */
    "message_halt.c_trigger",                   /**< Event message_halt.c_trigger. */
    "data_halt.c_p6YHW6KZL68HQ",                /**< Event data_halt.c_p6YHW6KZL68HQ. */
...
/**
 * Execute code for event "component_ObjectDetector.c_pFDUHMFYFEZ1C".
 *
...
static BoolType execEvent0(void) {
    ...
    #if EVENT_OUTPUT
        controller_InfoEvent(component_ObjectDetector_c_pFDUHMFYFEZ1C_, TRUE);
    #endif

    component_ObjectDetector_ = _controller_object_found;

    #if EVENT_OUTPUT
        controller_InfoEvent(component_ObjectDetector_c_pFDUHMFYFEZ1C_, FALSE);
    #endif
    return TRUE;
}

/**
 * Execute code for event "component_ObjectDetector.c_pIT6QHH818CVU".
 *
 * @return Whether the event was performed.
 */
static BoolType execEvent1(void) {
    ...
    #if EVENT_OUTPUT
        controller_InfoEvent(component_ObjectDetector_c_pIT6QHH818CVU_, TRUE);
    #endif

    component_ObjectDetector_ = _controller_no_object;

    #if EVENT_OUTPUT
        controller_InfoEvent(component_ObjectDetector_c_pIT6QHH818CVU_, FALSE);
    #endif
    return TRUE;
}

/**
 * Execute code for event "data_halt.c_p6YHW6KZL68HQ".
 *
 * @return Whether the event was performed.
 */
static BoolType execEvent2(void) {
    BoolType guard = ((data_halt_) == (_controller_none)) || ((data_halt_) == (_controller_data_p6WJ9Q5EL6S92));
    if (!guard) return FALSE;

    #if EVENT_OUTPUT
        controller_InfoEvent(data_halt_c_p6YHW6KZL68HQ_, TRUE);
    #endif

    if ((data_halt_) == (_controller_none)) {
        data_halt_ = _controller_data_p6WJ9Q5EL6S92;
    } else if ((data_halt_) == (_controller_data_p6WJ9Q5EL6S92)) {
        data_halt_ = _controller_data_p6WJ9Q5EL6S92;
    }

    #if EVENT_OUTPUT
        controller_InfoEvent(data_halt_c_p6YHW6KZL68HQ_, FALSE);
    #endif
    return TRUE;
}

/**
 * Execute code for event "data_move.c_p1HIPH7IC9U78".
 *
 * @return Whether the event was performed.
 */
static BoolType execEvent3(void) {
    BoolType guard = ((data_move_) == (_controller_none)) || ((data_move_) == (_controller_data_p09VPMA0YF0LE));
    if (!guard) return FALSE;

    #if EVENT_OUTPUT
        controller_InfoEvent(data_move_c_p1HIPH7IC9U78_, TRUE);
    #endif

    if ((data_move_) == (_controller_none)) {
        data_move_ = _controller_data_p09VPMA0YF0LE;
    } else if ((data_move_) == (_controller_data_p09VPMA0YF0LE)) {
        data_move_ = _controller_data_p09VPMA0YF0LE;
    }

    #if EVENT_OUTPUT
        controller_InfoEvent(data_move_c_p1HIPH7IC9U78_, FALSE);
    #endif
    return TRUE;
}
...
static void PerformEvents(void) {
    ...
    for (;;) {
        ...
        if (execEvent0()) continue;  /* (Try to) perform event "component_ObjectDetector.c_pFDUHMFYFEZ1C". */
        if (execEvent1()) continue;  /* (Try to) perform event "component_ObjectDetector.c_pIT6QHH818CVU". */
        if (execEvent2()) continue;  /* (Try to) perform event "data_halt.c_p6YHW6KZL68HQ". */
        if (execEvent3()) continue;  /* (Try to) perform event "data_move.c_p1HIPH7IC9U78". */
...
BoolType controller_EnginePerformEvent(controller_Event_ event) {
    switch (event) {
        case component_ObjectDetector_c_pFDUHMFYFEZ1C_:
            return execEvent0();
        case component_ObjectDetector_c_pIT6QHH818CVU_:
            return execEvent1();
        case data_halt_c_p6YHW6KZL68HQ_:
            return execEvent2();
        case data_move_c_p1HIPH7IC9U78_:
            return execEvent3();
