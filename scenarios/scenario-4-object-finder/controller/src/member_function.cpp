...
public:	
	...
	void call_message_rotate_left() {
		fprintf(stderr, "[debug] Rotate left\n");
	...
	void callback_message_object_count(const darknet_ros_msgs::msg::ObjectCount::SharedPtr msg) {
		message_object_count_i_response_count_ = msg->count;
	...
	void call_message_move() {
		...
		if (data_move_ == _controller_data_pO6JO6QW5TBOU) {
			value.linear.x = 0.5;
			value.angular.z = 0.0;
		}
	...
	void call_message_halt() {
		...
		if (data_halt_ == _controller_data_pLV2V0NPQH35K) {
			value.linear.x = 0.0;
			value.angular.z = 0.0;
		}
...
private:
	...
	void tick() {
		int nOfDataEvents = 2;
		      controller_Event_ data_events[2] = { data_move_c_p9ON15CK7WIV2_,data_halt_c_pZQW1X9M3P0X5_ };
		...
		int nOfControllableEvents = 6;
		      controller_Event_ controllable_events[6] = { component_ObjectDetector_c_pU6SJARKG2TD7_,component_ObjectDetector_c_pEQ73M2PZ6GC9_,message_rotate_left_c_trigger_,message_rotate_right_c_trigger_,message_move_c_trigger_,message_halt_c_trigger_ };
