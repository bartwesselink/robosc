...
public:	
	...
	void call_message_move() {
		...
		if (data_move_ == _controller_data_pCQX6CM3BUPL0) {
			value.linear.x = 0.5;
			value.angular.z = 0.0;
		}
	...
	void call_message_halt() {
		...
		if (data_halt_ == _controller_data_p7OE2BQGG0KUT) {
			value.linear.x = 0.0;
			value.angular.z = 0.0;
		}
...
private:
	...
	void tick() {
		int nOfDataEvents = 2;
		      controller_Event_ data_events[2] = { data_move_c_pUTYLDIZONYOY_,data_halt_c_pHVTLEI7SGDMN_ };
		...
		int nOfControllableEvents = 6;
		      controller_Event_ controllable_events[6] = { component_ObjectDetector_c_pKMRFS5Y9MB7Z_,component_ObjectDetector_c_p2YZHOT00LPXZ_,message_rotate_left_c_trigger_,message_rotate_right_c_trigger_,message_move_c_trigger_,message_halt_c_trigger_ };
