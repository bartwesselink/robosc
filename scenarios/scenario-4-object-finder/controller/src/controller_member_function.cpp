...
public:	
	...
	void call_message_move() {
		...
		if (data_move_ == _controller_data_pWRV7WDRV9AWV) {
			value.linear.x = 0.5;
			value.angular.z = 0.0;
		}
	...
	void call_message_halt() {
		...
		if (data_halt_ == _controller_data_pXE2TUBMKC6JJ) {
			value.linear.x = 0.0;
			value.angular.z = 0.0;
		}
...
private:
	...
	void tick() {
		int nOfDataEvents = 2;
		      controller_Event_ data_events[2] = { data_move_c_pM1UN2OYGAKLT_,data_halt_c_pX1QS0MF35RKJ_ };
		...
		int nOfControllableEvents = 6;
		      controller_Event_ controllable_events[6] = { component_ObjectDetector_c_pZ41MGGRSOLP8_,component_ObjectDetector_c_pFD97JMR4P7BW_,message_rotate_left_c_trigger_,message_rotate_right_c_trigger_,message_move_c_trigger_,message_halt_c_trigger_ };
