...
public:	
	...
	void call_message_move() {
		...
		if (data_move_ == _controller_data_pXQ126KPL2J3Q) {
			value.linear.x = 0.6;
	...
	void call_message_halt() {
		...
		if (data_halt_ == _controller_data_pD4IR9513ODHE) {
			value.linear.x = 0.0;
			value.angular.z = 0.0;
		}
...
private:
	...
	void tick() {
		int nOfDataEvents = 2;
		      controller_Event_ data_events[2] = { data_move_c_pXKXTO3QW1ZOR_,data_halt_c_pHH8UCZ6MO5KK_ };
