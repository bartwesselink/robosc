...
public:	
	...
	void call_message_move() {
		...
		if (data_move_ == _controller_data_pOAWF8LCBIENY) {
			value.linear.x = 0.6;
	...
	void call_message_halt() {
		...
		if (data_halt_ == _controller_data_p3HE1WVOR57YN) {
			value.linear.x = 0.0;
			value.angular.z = 0.0;
		}
...
private:
	...
	void tick() {
		int nOfDataEvents = 2;
		      controller_Event_ data_events[2] = { data_move_c_pSIUJBIUI0URX_,data_halt_c_pXI4SPSLA4SJ0_ };
