...
const char *controller_event_names[] = {
    ...
    "data_navigate.c_pJ2NGB46ZD8A8",   /**< Event data_navigate.c_pJ2NGB46ZD8A8. */
...
/**
 * Execute code for event "data_navigate.c_pJ2NGB46ZD8A8".
 *
 * @return Whether the event was performed.
 */
static BoolType execEvent6(void) {
    BoolType guard = ((data_navigate_) == (_controller_none)) || ((data_navigate_) == (_controller_data_pQWNL92YSD3BC));
    if (!guard) return FALSE;

    #if EVENT_OUTPUT
        controller_InfoEvent(data_navigate_c_pJ2NGB46ZD8A8_, TRUE);
    #endif

    if ((data_navigate_) == (_controller_none)) {
        data_navigate_ = _controller_data_pQWNL92YSD3BC;
    } else if ((data_navigate_) == (_controller_data_pQWNL92YSD3BC)) {
        data_navigate_ = _controller_data_pQWNL92YSD3BC;
    }

    #if EVENT_OUTPUT
        controller_InfoEvent(data_navigate_c_pJ2NGB46ZD8A8_, FALSE);
    #endif
    return TRUE;
}
...
static void PerformEvents(void) {
    ...
    for (;;) {
        ...
        if (execEvent6()) continue;  /* (Try to) perform event "data_navigate.c_pJ2NGB46ZD8A8". */
...
BoolType controller_EnginePerformEvent(controller_Event_ event) {
    switch (event) {
        ...
        case action_navigate_u_response_:
            return execEvent5();
        case data_navigate_c_pJ2NGB46ZD8A8_:
            return execEvent6();
