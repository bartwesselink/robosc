...
const char *controller_event_names[] = {
    ...
    "data_navigate.c_pU4E1Z2AILZWF",   /**< Event data_navigate.c_pU4E1Z2AILZWF. */
...
/**
 * Execute code for event "data_navigate.c_pU4E1Z2AILZWF".
 *
 * @return Whether the event was performed.
 */
static BoolType execEvent6(void) {
    BoolType guard = ((data_navigate_) == (_controller_none)) || ((data_navigate_) == (_controller_data_pAZSNM9FT2GVI));
    if (!guard) return FALSE;

    #if EVENT_OUTPUT
        controller_InfoEvent(data_navigate_c_pU4E1Z2AILZWF_, TRUE);
    #endif

    if ((data_navigate_) == (_controller_none)) {
        data_navigate_ = _controller_data_pAZSNM9FT2GVI;
    } else if ((data_navigate_) == (_controller_data_pAZSNM9FT2GVI)) {
        data_navigate_ = _controller_data_pAZSNM9FT2GVI;
    }

    #if EVENT_OUTPUT
        controller_InfoEvent(data_navigate_c_pU4E1Z2AILZWF_, FALSE);
    #endif
    return TRUE;
}
...
static void PerformEvents(void) {
    ...
    for (;;) {
        ...
        if (execEvent6()) continue;  /* (Try to) perform event "data_navigate.c_pU4E1Z2AILZWF". */
...
BoolType controller_EnginePerformEvent(controller_Event_ event) {
    switch (event) {
        ...
        case action_navigate_u_response_:
            return execEvent5();
        case data_navigate_c_pU4E1Z2AILZWF_:
            return execEvent6();
